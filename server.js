...
app.post('/api/analises', async (req, res) => {
  try {
    ...
    `, [geojsonStr]);
    // Nomes de rios via schema rio_nomes (auto-descoberta da tabela/coluna)
    const rioTablesRes = await safeQuery(`
      ...
      WHERE t.table_schema = 'rio_nomes'
      GROUP BY t.table_name
    `, []);
    console.log('[DEBUG] rio_nomes schema:', JSON.stringify(rioTablesRes.rows));
    let rioTable = null, rioNameCol = null, rioGeomCol = null;
    const nameCandidates = ['nome','noriocomp','nm_rio','nome_rio','nm','name','rio','descricao'];
    const geomCandidates = ['geom','the_geom','geometry','shape'];
    for (const row of (rioTablesRes.rows || [])) {
      ...
      if (g && n) { rioTable = row.table_name; rioGeomCol = g; rioNameCol = n; break; }
    }
    let nomesRes = { rows: [] };
    if (rioTable && rioNameCol && rioGeomCol) {
      console.log('[DEBUG] using rio_nomes.' + rioTable + ' (name=' + rioNameCol + ', geom=' + rioGeomCol + ')');
      nomesRes = await safeQuery(`
        SELECT DISTINCT "${rioNameCol}" AS nome
        FROM rio_nomes."${rioTable}"
        WHERE ST_DWithin(
